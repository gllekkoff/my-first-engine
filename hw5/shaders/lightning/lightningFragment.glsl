...
vec3 getNormalFromMap() {
    ...
    vec3 tangentNormal = texture(normalMap, vs_out.Tex).xyz * 2.0 - 1.0;
...
vec3 getDiffuseColor() {
    if (useFloorTiling) {
        vec2 tiled = vs_out.FragPos.xz / max(floorTileScale, 0.0001);
        return texture(floorTexture, tiled).rgb;
    }

    if (useModelTexture) {
        return texture(material.texture_diffuse1, vs_out.Tex).rgb;
    }
...
vec3 getSpecularColor() {
    if (useModelTexture) {
        return material.specular * texture(material.texture_specular1, vs_out.Tex).rgb;
    }
...
void main() {
    ...
    float shadow = ShadowCalculation(vs_out.FragPosLightSpace, N, normalize(-dirLight.direction));
    color += CalcDirLight(dirLight, N, V) * (1.0 - shadow);

    color += CalcPointLight(pointLights[0], N, V, vs_out.FragPos);
    color += CalcPointLight(pointLights[1], N, V, vs_out.FragPos);
    color += CalcSpotLight(spotLight, N, V, vs_out.FragPos);
