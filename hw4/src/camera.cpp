...
void Camera::ProcessKeyboard(const DIRECTIONS direction, const float deltaTime) {
    ...
    if (direction == FRONT) Pos += Front * velocity;
    if (direction == BACK) Pos -= Front * velocity;
    if (direction == LEFT) Pos -= Right * velocity;
    if (direction == RIGHT) Pos += Right * velocity;
    if (direction == UP) Pos += glm::normalize(WorldUp) * velocity;
    if (direction == DOWN) Pos -= glm::normalize(WorldUp) * velocity;
}
